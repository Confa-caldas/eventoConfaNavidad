...
import { User, UserOrCompany, Token, ValidateToken } from '../../interfaces/user.interface';
...
export class WelcomeComponent implements OnInit {
  ...
  constructor(
    ...
    this.userOrCompany = (localStorage.getItem('userOrCompany') !== '') ? JSON.parse(localStorage.getItem('userOrCompany')) : null;
  ...
  loadCurrentUser() {
    this.validateLogin()
      .then(() => {
        ...
        if (currentUser) {
          this.login = true;
          this.utilitiesService.fullNameUser = currentUser.primerNombre + " " + currentUser.segundoNombre + " " + currentUser.primerApellido + " " + currentUser.segundoApellido;
  ...
  private validateLogin() {
    ...
    return new Promise((resolve, reject) => {
      this.authenticationService.login(ptoken.token)
        .pipe(first())
  ...
  loadUserWithoutAuth(gtoken: Token) {
    this.validateUserWithoutAuth(gtoken)
      .then(() => {
        ...
        if (nasfaUser) {
          this.login = true;
          this.utilitiesService.fullNameUser = nasfaUser.primerNombre + " " + nasfaUser.segundoNombre + " " + nasfaUser.primerApellido + " " + nasfaUser.segundoApellido;
  ...
  validateUserWithoutAuth(gtoken: Token) {
    let user: UserOrCompany = JSON.parse(localStorage.getItem('userOrCompany'));
    this.loading = true;

    return new Promise((resolve, reject) => {
      this.authenticationService.validateToken(gtoken.token)
        .pipe(first())
        .subscribe((response: ValidateToken) => {
          // console.log(response);
          if (response.valido) {

            this.authenticationService.consultUserInformationNASFA(user.document)
              .pipe(first())
              .subscribe((response: User) => {
                // console.log(response);
